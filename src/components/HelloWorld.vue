    ...
    <div class="content">
      <ul v-loading.body="loading">
        <li v-for="(item, index) in msg" :key="item.id"
            :class="[index % 2 === 0?'orange':'white']"
            :title="item.title"
            @click="click(item.id)">
          <i class="el-icon-arrow-right" :class="[index % 2 === 0?'i-white':'i-orange']" style="font-weight: bold;"></i>
...
export default {
  ...
  mounted () {
    this.loading = true
    // 取cookie
    this.openSimple = !this.$cookie.get('bestfcb')
    let url = Host + 'bestFirstChineseBelow'
    this.post(url, '', res => {
      this.loading = false
      if (res.status) {
        this.info = res.info
  ...
  methods: {
    ...
    bClose () {
      ...
      this.$cookie.set('bestfcb', 'Helloworld', parseInt(this.info.adInfo.interval))
    }
  }
}
</script>
